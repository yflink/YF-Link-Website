...
class RewardPools extends Component {
  ...
  render() {
    ...
    const { account, loading, modalOpen } = this.state;
      ...
      <div className={classes.root}>
        ...
        <div className={classes.rewardPools}>
          <Typography variant={"h3"} className={classes.title} noWrap>
            Which tokens would you like to stake?
          </Typography>
          {this.renderRewards()}
        </div>
  ...
  renderRewardPool = (rewardPool) => {
    ...
    return (
      <div className={classes.rewardPoolContainer} key={rewardPool.id}>
        <Typography variant="h3" className={classes.poolTitle}>
          {rewardPool.title}
        </Typography>
        <Typography variant="h5" className={classes.poolName}>
          {rewardPool.name}
        </Typography>
        <Typography variant="h4" className={classes.poolWebsite}>
          <a href={rewardPool.link} target="_blank" rel="noopener noreferrer">
            {rewardPool.website}
          </a>
        </Typography>
        <Button
          className={classes.poolInstructions}
          variant="outlined"
          color="secondary"
          onClick={() => window.open(rewardPool.instructionsLink, "_blank")}
        >
        ...
        >
          <Typography variant={"h4"}>
            {rewardPool.depositsEnabled === false
              ? "Rewards exhausted"
              : openingSoon
              ? "Opening Soon"
              : rewardPool.id.startsWith("gov")
              ? "Stake"
              : "Mine $YFL"}
          </Typography>
        </Button>
        {openingSoon && (
          <Typography className={classes.timestamp}>
            Opens {rewardPool.startDate.utc().format("D MMM, HH:mm")} UTC (
            {rewardPool.startDate.fromNow(true)})
          </Typography>
        )}
      </div>
    );
